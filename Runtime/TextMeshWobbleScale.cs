...
namespace Doto.RichTMP
{
    ...
    public class TextMeshWobbleScale : MonoBehaviour
    {
        ...
        private async UniTask AnimateText()
        {
            ...
            while (!_cancellationToken.IsCancellationRequested)
            {
                ...
                for (int i = 0; i < characterCount; i++)
                {
                    ...
                    Vector3 wobbleOffset = new(
                        Mathf.Sin(time + phase),
                        Mathf.Cos(time * 0.8f + phase),
                        0f
                    );
                    wobbleOffset *= shakeRadius;

                    float scale = 1f + Mathf.Sin(time * 1.2f + phase) * scaleAmplitude;
                    Vector3 center = (
                        _baseVertices[materialIndex][vertexIndex] +
                        _baseVertices[materialIndex][vertexIndex + 1] +
                        _baseVertices[materialIndex][vertexIndex + 2] +
                        _baseVertices[materialIndex][vertexIndex + 3]
                    ) / 4f;

                    for (int j = 0; j < 4; j++)
                    {
                        Vector3 baseVertex = _baseVertices[materialIndex][vertexIndex + j];
                        Vector3 scaledOffset = (baseVertex - center) * scale;
                        vertices[vertexIndex + j] = center + scaledOffset + wobbleOffset;
                    }
                }
